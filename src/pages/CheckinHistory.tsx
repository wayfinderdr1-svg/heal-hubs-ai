...
const CheckinHistory = () => {
  ...
  const fetchCheckins = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('daily_checkins').select('*').eq('user_id', user.id).gte('checkin_date', startDate.toISOString().split('T')[0]).order('checkin_date', {
      ascending: true
    });
    if (error) {
  ...
  const getMostCommonGoals = () => {
    ...
    return Object.entries(goalCounts).sort(([, a], [, b]) => b - a).slice(0, 5).map(([goal, count]) => ({
      name: goal,
      value: count
    }));
  };
      ...
      <main className="pb-12">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 bg-cyan-950">
          <div className="max-w-7xl mx-auto">
            ...
            {checkins.length === 0 ? <Card className="text-center py-12">
                <CardContent>
                ...
                {/* Average Stats */}
                {averages && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
                    <Card>
                ...
                {commonGoals.length > 0 && <Card>
                    ...
                    <CardContent>
                      <div className="h-56 sm:h-64 md:h-72 lg:h-80">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie data={commonGoals} cx="50%" cy="50%" labelLine={false} label={({
                        name,
                        percent
                      }) => `${name} (${(percent * 100).toFixed(0)}%)`} outerRadius={window.innerWidth < 768 ? 60 : 80} fill="#8884d8" dataKey="value">
                              {commonGoals.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                            </Pie>
                            <Tooltip />
                          </PieChart>
                        </ResponsiveContainer>
